...
def prepare_dogon_message(idx: int):
    ...
    dg_message += (f'<blockquote>'
                   ...
                   f'<emoji id="5431577498364158238">📊</emoji> <b><i>Разница пунктов: '
...
def dop_dogon_message():
    ...
    dg_message = ('<b><i><emoji id="5472146462362048818">💡</emoji>'
                  'Условия повторного входа в рынок:</i></b>\n\n')
...
def minus_dogon_message():
    ...
    s_message += (f'<blockquote>'
                  ...
                  f'</blockquote>\n\n')
    s_message += f'<i>Итог прогноза: <b>минус</b></i> <emoji id="5019523782004441717">❌</emoji>\n\n'
    return s_message


def main_bug_message():  # Сообщение о сбое сервера
    f_message = ('<b><i>Сейчас ведутся технические улучшения нашего торгового алгоритма '
                 '<emoji id="5188217332748527444">🔍</emoji></i></b>\n\n')
    f_message += ('<b><i><emoji id="5472189549473963781">🙏</emoji>'
                  ' МОЛИТВА ФИНАНСОВОГО ТРЕЙДЕРА '
                  '<emoji id="5472189549473963781">🙏</emoji></i></b>\n\n')
    ...
    f_message += ('<i><emoji id="5377844313575150051">📎</emoji> '
                  '<b>ВАЖНО:</b> Мы скоро возобновим свою работу!</i>')
    return f_message
...
# Футер «ВАЖНО» в конце каждого плюс-поста
pl_mes = ('<blockquote>'
...
def _plus_earn(amount: str) -> str:
    """Единое тело плюс-серии: меняется только сумма заработка."""
    return (f'<i>При заходе в каждый прогноз, вы могли бы заработать около — </i>'
            f'<i><b><u>{amount}</u> ₽ чистыми</b>, при выплате по активу </i>'
            f'<i><b><u>от 75% — 85%</u></b>! Помните, что инвестирование небольших сумм и '
            f'использование риск и мани менеджмента могут генерировать ваш стабильный доход </i>'
            f'<i><b><u>от 5.ООО</u> ₽ </b> — </i>'
            f'<i><b><u>1.ООО.ООО</u> ₽</b> ежедневно!</i> {_PLUS_BOLT}\n\n')
...
def dop_plus10_message():
    ...
    message += ('<emoji id="5397569414738485822">🟡</emoji> '
                '<b><i>Ссылка на чат-бот для отзывов: </i></b>'
...
def weekend_message():
    message = ('<b><i>Завершение торговой недели в закрытом сообществе '
               '<emoji id="5190498849440931467">👨‍💻</emoji></i></b>\n\n')
    ...
    message += ('<blockquote>'
                ...
                '</blockquote>\n\n')
    message += ('<i><emoji id="5377844313575150051">📎</emoji><b> ВАЖНАЯ ЗАМЕТКА: </b>Наше сообщество '
