...
design. Перенос рабочего модуля из проекта Screens/BinoOptions — без внешних зависимостей, чистый
stdlib (threading/socket/select).

ГДЕ ЛЕЖИТ. В `classes/`, а не в `settings/` (переехал 17-09-2026 по аудиту). Это работающий
сетевой сервер — сокеты, потоки, `select`, — а не конфигурация; §2 стандарта говорит про это
прямо: «Сетевые механизмы — не конфиг: им место в classes/, а не в settings/». Подбор прокси
(какой из пула брать, кого банить) остаётся в `settings/proxy.py`: это политика над данными из
БД, и конфигом она быть как раз может.

ДВА КЛАССА В ОДНОМ ФАЙЛЕ — намеренно (§1, нюанс 2): `LocalProxyServer` и `ProxyAuthThread` —
ОДИН механизм, сервер и его рабочий поток. Врозь файл не читается: поток бессмыслен без того,
кто порождает его на каждое принятое соединение, а сервер без потока не делает ничего.
"""
