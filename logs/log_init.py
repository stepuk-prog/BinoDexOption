...
async def close_telegram_bot():
    ...
    # Один проход лупа ПЕРЕД сбором набора: запись ИЗ ТРЕДА попадает в emit через
    # loop.call_soon_threadsafe, то есть её задача отправки ещё не создана и в наборе её нет —
    # без этого шага уборка такой алерт не увидит. Проверено 15-09-2026: с проходом лупа
    # алерт из треда доставлен, без него — потерян.
