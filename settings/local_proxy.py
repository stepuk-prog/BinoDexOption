"""Локальный прокси-релей с авторизацией для OTC-режима (binodex).

Firefox под Playwright ненадёжно жуёт авторизацию upstream-прокси напрямую (proxy.username/
password), поэтому поднимаем локальный релей на 127.0.0.1:<рандом-порт>, который подставляет
заголовок `Proxy-Authorization: Basic ...` и форвардит на удалённый HTTP-прокси из
settings.proxy_data. Браузеру отдаём адрес локального релея (без авторизации).

Перенос рабочего модуля из проекта Screens (тот же приём для Pocket Option) — без внешних
зависимостей, чистый stdlib (threading/socket/select).
"""
