...
def _fatal_bootstrap(db: str, error: Exception) -> NoReturn:
    ...
    Импорт fatal ЛОКАЛЬНЫЙ, но НЕ из-за круга — круга здесь нет: к этому моменту
    `settings.fatal` уже загружен по цепочке constant → fatal → env → database_config →
    _bootstrap (проверено). Локальным он оставлен, чтобы обработчик отказа не зависел от
    порядка импортов, если модуль позовут напрямую — из тула или диагностики.
    """
