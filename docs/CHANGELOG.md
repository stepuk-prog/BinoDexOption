...
### 2026-08-08 — Fixed: FIN не закрывает неделю на минусовом опционе
- **`apps/app.py` `exit_main`.** `plus` в `MainResult` теперь честный итог опциона
  (`bool(option_data.plus)`), а не «отработали без сбоя»: раньше `plus = True` ставился на любом
  чистом завершении, включая минус после всех догонов (`main_app.py`, `channel_mess=False`).
- **`main.py`, ветка выходных FIN.** Из-за этого выход на выходные срабатывал и после минуса —
  последним постом недели мог быть минус. Теперь `not res_option.plus` (минус ИЛИ сбой опциона) →
  бот НЕ выходит, ждёт паузу и берёт следующий опцион, пока не закроется плюсом; тогда шлёт
  `end_week.png`, пишет `status=false` и выходит. Логика инварианта не тронута (status=false
  по-прежнему ставит только плановый weekend-выход binary), других потребителей `plus` нет.
