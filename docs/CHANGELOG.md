...
### 2026-08-04 — Кадр OTC: своя подложка слоем в браузере + индикатор графика
- **Подложка кадра теперь рисуется браузером** (`apps/otc_app.py`): свой слой `#bino_bg` (fixed,
  `z-index:-1`, промоутнут `translateZ(0)`) позиционируется ровно по боксу канваса и заливается
  цветом `settings.constant.bg_otc_color` (чёрный — у binodex свой фон тёмно-синий). Кадр снимается
  `element.screenshot` по тому же боксу. Штатный фон binodex (`.wrap_bg`) как был выключен за
  аккаунтом, так и остался. Бонусом кадр похудел ~500 КБ → ~56 КБ (сплошной фон жмётся).
  *Если понадобится картинка вместо цвета:* слой умеет `background-image` — вернуть `data-URI`
  (ассет печь под бокс канваса 1470×870; рецепт затемнения — cover → множитель яркости → гамма >1,
  гамма давит светлый дым, а не тени).
- **Убран PIL-композит** — `_load_globe`/`_canvas_alpha`/`_matte_label`/`_label_cutout`/
  `_build_label_cutout` и файл `pictures/globe_otc.png` (~140 строк). Ярлык пары теперь попадает в
  кадр сам (он HTML внутри бокса канваса), вырезка по разнице кадров больше не нужна — с ней ушёл и
  единственный слепой `wait_for_timeout(150)` в OTC-флоу.
- **Цена этого решения измерена** (`docs/BINODEX_CPU.md`, перезамер 2026-08-04): +4…6 пт CPU
  (23%→27%) и +85 мс на кадр. Июльские «фон = +56 пт» относились к конфигурации без off-zone.
- **Проверка «канвас пуст»** переехала с гистограммы альфы `toDataURL` на JS-пробу `_CANVAS_FILL_JS`
  (даунсэмпл 64×40 через `drawImage`, без PNG-энкода) — кадр больше не приходит в Python в виде
  пикселей канваса, а проба стала дешевле прежней.
- **off-zone: второй проход «чистка кадра»** (`_HIDE_OFFZONE_JS`). Кадр теперь клип по боксу, поэтому
  видимый HTML внутри бокса попадал бы в пост (тулбар `graph_pair_setting`, селектор масштаба,
  боковые ручки — их возвращает `show(pl.parentElement)` ради ярлыка). Прячем всё видимое,
  пересекающее бокс, кроме цепочки канваса, ярлыка, подложки и `#setup_settings_open` (детект кук).
  Правило геометрическое, не по классам binodex. Грабли: у части UI висит `transition:.5s`, и
  `visibility` АНИМИРУЕТСЯ — computed остаётся `visible` ещё полсекунды, спрятанное успевало попасть
  в кадр; поэтому вместе с `visibility` ставим `transition:none` (снимается в `_CLEAR_OFFZONE_JS`).
- **Индикатор графика** (`apply_indicator`, вызов в `init_otc` до off-zone): binodex держит набор в
  `localStorage` (`indicators/chart-<n>`), а контекст поднимается из storage_state в БД, где его может
  не быть → включаем на каждом старте браузера, идемпотентно (уже включён → без кликов). Имя и
  селектор пункта — в БД: `settings.binodex_settings.indicator_name` (= `Whale Absorption`) и
  `indicator_item` (= `button.chart_indicator`, выбор ПО ТЕКСТУ). Пункт кликается при снятом
  off-zone — модалка индикаторов вне зоны канваса.
- **Удалён** `pictures/globe_otc.png` — ассет старого композита больше не используется.
