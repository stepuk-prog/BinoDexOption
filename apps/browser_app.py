...
async def clear_zone_overlays(page: Page, zone_selector: str, attempts: int = 3) -> None:
    ...
    (11-09-2026 — «Теперь можно перемещать таблицы индикаторов…» уехало в канал). Кнопку жмём только если она ВНЕ зоны-контейнера и у неё есть плавающий предок
    (position:fixed — без требования z-index, либо absolute с z-index > 0), иначе можно
    попасть по кнопке самого графика.
