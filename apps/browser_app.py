...
(selector) => {
  ...
  // узлы, у которых закрывашки НЕТ, — ИЛИ те, чью закрывашку мы уже жали без толку (см. TRIED).
  //
  // TRIED и есть лечение оставшейся дыры (аудит 17-09-2026, п.3.2): ветка 1 рапортовала
  // 'closed' по ФАКТУ КЛИКА, не проверяя, что окно исчезло, а ветка 2 пропускала всё, у чего
  // закрывашка есть. Окно с декоративным/сломанным крестиком поэтому забирало все три attempts
  // и ОСТАВАЛОСЬ В КАДРЕ, а ветка диагностики (проба девяти точек) не выполнялась ни разу —
  // то есть в логе не оставалось ни строки о том, что реально лежит поверх графика. Теперь
  // плавающий предок нажатой кнопки помечается, второй раз мы его не жмём (бюджет не жжём), и
  // на следующем проходе он достаётся удалению.
  const TRIED = 'data-zone-close-tried';
  // Помеченные ИМЕННО СЕЙЧАС: удалять их в этом же проходе нельзя — клик мы только что
  // сделали, странице надо дать перерисоваться. React ещё владеет узлом, и выдёргивать его
  // из-под живого обработчика — не то же самое, что снять окно, которое клик не берёт.
  // Удаление достаётся СЛЕДУЮЩЕМУ проходу, если окно к тому моменту на месте.
  const markedNow = new Set();
  ...
  for (const btn of document.querySelectorAll(CANDIDATES)) {
    ...
    // Этот крестик уже жали на прошлом проходе, а окно на месте — значит он не работает.
    // Второй раз не жмём (иначе снова 'acted' и снова съеденная попытка): узел помечен,
    // его заберёт ветка удаления ниже.
    if (el.hasAttribute(TRIED)) continue;
    // Жмём ПЕРВУЮ подошедшую и дальше по кнопкам не идём: слепо кликать всё, что похоже на
    // закрывашку, опаснее, чем оставить второе окно следующему проходу. Помечаем ПЛАВАЮЩЕГО
    // ПРЕДКА (само окно), а не кнопку: если клик сработал, узел исчезнет вместе с меткой, и
    // заново появившееся окно будет для нас новым.
    try {
      el.setAttribute(TRIED, '1'); markedNow.add(el);
      btn.click(); acted.closed = txt || 'close-attr'; break;
    } catch (e) {}
  }
  ...
  //   • закрывашки внутри НЕТ — ЛИБО она есть, но её уже жали и окно не ушло (метка TRIED):
  //     тогда это не «дело ветки 1», а сломанный крестик, и окно снимается удалением;
